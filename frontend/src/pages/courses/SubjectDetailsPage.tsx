...
export default function SubjectDetailsPage() {
  ...
  const { mutate: mutateProgress } = useCourseProgress();

  const { data: stats } = useStudentStats();

  const { coursesProgress } = stats?.data || {};

  // initialize from existing progress data
  const isCompleted = (courseId: string) =>
    coursesProgress?.find((c: coursesProgress) => c.id === courseId)?.completed;
  if (isLoading) {
    return (
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-linear-to-t from-[#D6DAF5] to-[#fafafa] rounded-[24px] p-8 shadow-lg border border-white max-w-4xl mx-auto">
              ...
              <GlassButton
                ...
                onClick={() => {
                  mutateProgress({
                    ...
                    progress: isCompleted(subject._id) ? 0 : 100,
                  });
                }}
              >
