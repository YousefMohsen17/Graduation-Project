...
export default function SignupForm() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormData({
      ...formData,
      [id]: id === "academicYear" || id === "universityId"
        ? Number(value)
        : value
    });
  };
  ...
  return (
    <form className="w-full max-w-[643px]" onSubmit={(e) => {
      handleRegister(e)
    }}>
      ...
      <div className="mb-4">
        ...
        >
          Confirm Password
          <span className="flex gap-2 font-bold cursor-pointer" onClick={() => setShowConfirmPassword(!showConfirmPassword)}>
            {showConfirmPassword ? <Eye /> : <EyeOff />}
          </span>
        </label>
        <Input type={showConfirmPassword ? "text" : "password"} id="confirmPassword" value={formData.confirmPassword}
      ...
      <div className="mb-4">
        ...
        <select id="academicYear" className={`rounded-xl border ${errors.academicYear ? "border-2 border-red-500 outline-none " : "border border-[#050715] focus-visible:border-black focus-visible:ring-black"} bg-[#EAEDFA] shadow-[4px_4px_4px_4px_rgba(0,0,0,0.25)] py-2 px-4 w-full h-14`}
      ...
      <div className="mb-10">
        ...
        <Input type="number" id="universityId" value={formData.universityId}
      ...
      <ButtonLink
        ...
        children={isPending ? <LoaderCircle className="animate-spin " /> : "Sign Up"}
        disabled={isPending}
        type="submit"
      />
